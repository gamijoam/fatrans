...
export async function POST(
  ...
  try {
    ...
    const backendResponse = await fetch(
      `${BACKEND_URL}/api/v1/creditos/solicitudes/${encodeURIComponent(numero)}/evaluar`,
      {
        method: 'POST',
        headers: {
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json(
        { message: errorData.message || 'Error al evaluar solicitud' },
    ...
    console.error('Admin evaluar error:', error);
    return NextResponse.json(
