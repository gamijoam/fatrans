...
export async function GET(
  ...
  try {
    ...
    const backendResponse = await fetch(
      `${BACKEND_URL}/api/v1/documentos/constancia-afiliacion/${socioId}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json(
        { message: errorData.message || 'Error al generar constancia' },
    ...
    console.error('Constancia afiliacion error:', error);
    return NextResponse.json(
