// com.tufondo.documentospdf.application.usecase.GenerarContratoAdhesionUseCase
...
/**
 * Caso de uso para generar contratos de adhesión.
...
public class GenerarContratoAdhesionUseCase {
    ...
    private static final String BUCKET = "bucket-contratos";
    private static final int PRESIGNED_URL_MINUTES = 15;

    public DocumentoResponseDTO ejecutar(UUID solicitudId) {
        log.info("Generando contrato de adhesión para solicitudId={}", solicitudId);

        // 1. Obtener datos de la solicitud
        Map<String, Object> datosSolicitud = creditoQueryPort.obtenerDatosSolicitud(solicitudId);
        if (datosSolicitud == null) {
            throw new DocumentoNoEncontradoException("Solicitud no encontrada: " + solicitudId);
        }

        UUID socioId = (UUID) datosSolicitud.get("socioId");
        ...
        datosPdf.put("solicitud", datosSolicitud);
        datosPdf.put("socio", datosSocio);
        datosPdf.put("fechaContrato", LocalDate.now().format(DateTimeFormatter.ofPattern("dd 'de' MMMM 'de' yyyy")));

        // 4. Generar PDF
        byte[] pdfBytes;
        try {
            pdfBytes = pdfGeneratorPort.generarPdf(TipoDocumento.CONTRATO_ADHESION, datosPdf);
        } catch (Exception e) {
            log.error("Error al generar PDF de contrato", e);
            throw new GeneracionPDFException("Error al generar contrato: " + e.getMessage());
        }

        // 5. Escanear con ClamAV ANTES de firmar (seguridad: no firmar contenido malicioso)
        ...
        // 7. FIRMA DIGITAL RSA SHA-256 (requisito de seguridad CS-001)
        String firmaDigital = firmarPdf(pdfBytes);

        // 8. Subir a MinIO (bucket de contratos)
        String nombreArchivo = String.format("ContratoAdhesion_%s.pdf", solicitudId);
        String ruta = String.format("contratos/%s/%s", socioId, nombreArchivo);

        StoragePort.UploadResult uploadResult = storagePort.upload(BUCKET, ruta, pdfBytes, "application/pdf");

        // 9. Crear y guardar documento con firma digital
        ...
        log.info("Contrato de adhesion generado exitosamente: documentoId={}", documento.getId());
    ...
    /**
     * Firma digitalmente el PDF usando RSA SHA-256.
     * Requisito de seguridad CS-001.
     */
        ...
        try {
            if (clavePrivadaBase64 == null || clavePrivadaBase64.isEmpty()) {
                ...
                throw new FirmaDigitalException(
                        "DOC_005: No se puede generar CONTRATO. Contacte al administrador.");
            }
            ...
            throw new FirmaDigitalException("Error al firmar el contrato: " + e.getMessage(), e);
        }
    }
}
