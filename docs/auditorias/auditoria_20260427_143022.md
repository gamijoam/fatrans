...
**Impacto:**
Un atacante puede realizar **ataques de diccionario无限次** hasta encontrar la contraseña correcta. El sistema solo registra el intento fallido en auditoría pero **no bloquea ni limita**.
...
2. Adivinar tokens UUID v4 (已知弱点)
...
private String getClientIp(HttpServletRequest request) {
    ...
    if (xForwardedFor != null && !xForwardedFor.isEmpty()) {
        return xForwardedFor.split(",")[0].trim();  // ⚠️ Sin validar格式
    }
...
**Impacto:**
Un atacante puede falsificar la IP en los logs de auditoría para ocultar su origen real, dificultando la investigación forense y el追踪.
...
**Descripción:**
El controller de verificación no tiene protección específica contra abuse, relies únicamente en el filtro global que no cubre este endpoint.
